...
import raytracing.camera.Ray;
import raytracing.camera.TracePoint;
import raytracing.objet.Obj;
import raytracing.objet.Scene;
import raytracing.shader.QMCSamples;
import raytracing.shader.Sampling;
...
public class RenderChunk implements Runnable {
  ...
  public void renderPixels(Obj objectList[], int numSamples, int sampleCount, int numThreads) {
    for (int i = 0; i < numSamples; i++) {
      ...
      vignette = Sampling.limit(1.f - vignette * 0.5f, 0.f, 1.f);
