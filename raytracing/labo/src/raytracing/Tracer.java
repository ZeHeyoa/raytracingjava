...
import raytracing.camera.PolarCamera;
import raytracing.geometrie.PVector;
import raytracing.multithread.RenderChunk;
import raytracing.multithread.WorkQueue;
import raytracing.objet.Box;
import raytracing.objet.Obj;
import raytracing.objet.Scene;
import raytracing.shader.ParquetPlank;
...
public class Tracer extends PApplet {
	...
	Obj Obj_Materials[] = new Obj[] { new Obj(new Box(new PVector(-.5f, -.01f, -5f), new PVector(2.0f, 0.01f, 2f)),
			0.1f, 0.f, new ParquetPlank()) };

	Scene g_Scenes[] = new Scene[] { new Scene(Obj_Materials,
			new PolarCamera(new PVector(1.8213837f, 3.0841362f, -3.4671848f), -0.5062503f, -0.5520832f), false, true,
			false, true) };
	...
	private WorkQueue g_renderQueue;
	...
	public static void main(String args[]) {
		String[] a = { "Tracer" };
		PApplet.runSketch(a, Tracer.getInstance());
	...
	private float g_StartTime;

	private float GetTime() {
		return (float) millis() / 1000.f;
	}

	private int g_drawCount = 0;

	private void updateCamera() {
		g_prevCam = g_cam;
		g_cam = new Camera(g_Scene.getCamera().getPositionCamera(), new PVector(0.0f, 1.0f, 0.0f),
				g_Scene.getCamera().getDirectionCamera(), width, height, width);
		if (g_prevCam == null)
			g_prevCam = g_cam;
	}
