...
public class Shader {
  ...
  public static PVector lerp(PVector a, PVector b, float t) {
    return new PVector(PApplet.lerp(a.x, b.x, t), PApplet.lerp(a.y, b.y, t), PApplet.lerp(a.z, b.z, t));
  }
  ...
  public static float boxstep(float a, float b, float x) {
    return PApplet.constrain((x - a) / (b - a), 0.f, 1.f);
  }
  ...
  public static PVector LGParquetPlank(float s, float t, float pwidth) {
    ...
    if (swidth >= 1.)
      ...
      w = PApplet.constrain(boxstep(GWF - swidth, GWF, ss), Math.max(1 - GWF / swidth, 0), 1f)
          - PApplet.constrain(boxstep(1 - GWF - swidth, 1 - GWF, ss), 0, 2 * GWF / swidth);
    if (twidth >= 1)
      h = 1 - 2 * GHF;
    else
      h = PApplet.constrain(boxstep(GHF - twidth, GHF, tt), Math.max(1 - GHF / twidth, 0), 1)
          - PApplet.constrain(boxstep(1 - GHF - twidth, 1 - GHF, tt), 0, 2 * GHF / twidth);
